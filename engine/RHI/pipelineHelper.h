...
/* manually implemented ComputePipelineHelper */
...
public:
...

    {
        ...
        shaderModule = m_deviceHandle->createShaderModule(createInfo, allocationCallbacks);
...
{
public:
    ComputePipelineHelper(std::shared_ptr<vk::Device> device_, const vk::PipelineLayout &layout)
        : m_deviceHandle(device_), m_layout(layout) {}
    ~ComputePipelineHelper()
    {
        if (m_shader)
            m_deviceHandle->destroy(m_shader, allocationCallbacks);
    }

    void setLayout(const vk::PipelineLayout &layout) { m_layout = layout; }
                   ...
                   const char *entryPoint = "main")
    {
        ...
        createInfo.pCode = reinterpret_cast<const uint32_t *>(code.data());
        m_shader = m_deviceHandle->createShaderModule(createInfo, allocationCallbacks);

        m_stages.setStage(vk::ShaderStageFlagBits::eCompute)
            .setModule(m_shader)
            .setPName(entryPoint);
    }
    ...
    vk::Pipeline createPipeline(const vk::PipelineCache &cache)
    {
        ...
        createInfo.setLayout(m_layout).setStage(m_stages);
        auto [state, res] = m_deviceHandle->createComputePipeline(cache, createInfo, allocationCallbacks);
        vk::resultCheck(state, "[Vulkan] Failed to create compute pipeline.\n");
        return res;
    }
...
protected:
    ...
    vk::PipelineShaderStageCreateInfo m_stages{};
